...
$current_module_strings = return_module_language($current_language, 'it_serviceElectronic');
...
class Popup_Picker
{
    ...
    public function process_page()
    {
        ...
        foreach ($servicesRels as $relMod => $beanName) {
        ...
        foreach ($focus->get_linked_fields() as $field => $def) {
            if ($focus->load_relationship($field)) {
                ...
                if (array_key_exists($relTable, $servicesRels)) {
                    $varname = 'focus_' . $relTable . '_list';
                    ${$varname} =
                        sugarArrayMerge(${$varname}, $focus->get_linked_beans($field, $servicesRels[$relTable]));
                }
            }
        }
        ...
        return $template->fetch('modules/it_Services/tpls/PopupBody.tpl');
    }
